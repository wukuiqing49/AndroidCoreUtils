...
class PhotoPickerHelper private constructor(
     ...
     * 
     * @param maxItems 多选时的最大数量。系统多选合约要求该值大于 1，传入 1 时会自动按 2 处理。
    ...
    fun register(maxItems: Int = 9, onSelected: (List<Uri>) -> Unit): PhotoPickerHelper {
        ...
        val multipleMaxItems = maxItems.coerceAtLeast(2)
        ...
        multipleLauncher = when (registry) {
            is ComponentActivity -> registry.registerForActivityResult(ActivityResultContracts.PickMultipleVisualMedia(multipleMaxItems)) { uris ->
                processResult(uris)
            }
            is Fragment -> registry.registerForActivityResult(ActivityResultContracts.PickMultipleVisualMedia(multipleMaxItems)) { uris ->
                processResult(uris)
            }
            else -> null
        }
    ...
    /** 单选图片。 */
    fun launchImage() {
        launch(PickMediaType.IMAGE_ONLY, isMultiple = false)
    }

    /** 多选图片。 */
    fun launchImages() {
        launch(PickMediaType.IMAGE_ONLY, isMultiple = true)
    }

    /** 单选视频。 */
    fun launchVideo() {
        launch(PickMediaType.VIDEO_ONLY, isMultiple = false)
    }

    /** 图片/视频混选。 */
    fun launchImageAndVideo(isMultiple: Boolean = false) {
        launch(PickMediaType.IMAGE_AND_VIDEO, isMultiple)
    }
     ...
     * 
     * 注意：使用此方法必须已获得存储权限：
     * 低频选择照片/视频请优先使用系统 Photo Picker，避免申请照片/视频库读取权限。
     * 
     * @param type 选择类型
     * @param maxSize 最大文件大小 (Byte)
